/**
    Scenario: Demonstrates LINUX process creation via fork(), error handling using pointers,
              and synchronization using the wait() system call with multiple children.

    Details:
      This program spawns two child processes from a single parent process.
      Each child executes its task and exits, while the parent uses the wait()
      system call to intercept and clean up the termination status of whichever
...
int main() {
    ...
    if (childProcess1 > 0) {
        ...
        int status;
        // wait() blocks execution until ANY of the child processes terminates.
        // It returns the PID of the terminated child and populates the status variable.
        const pid_t terminatedChildId = wait(&status);
