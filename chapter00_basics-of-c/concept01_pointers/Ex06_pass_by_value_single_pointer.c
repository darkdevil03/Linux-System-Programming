/**
    Single Pointer (Pass-by-Value)

    Our code doesn't work (and will actually generate compiler warnings or errors) because of two major issues:
           -> a type mismatch and
           -> overwriting the local variable.

        Here is exactly what happens when we try to run the below code:

            1. The Type Mismatch
                In main, our variable ptr is an int * (a single pointer). When we use the address-of operator (&ptr),
                we are getting the address of a pointer, which makes it an int ** (a double pointer).

                However, our function is defined as void badAllocate(int *p). It is only expecting a single pointer.
            ...
            2. The Connection is Severed Immediately
                Let's pretend we forced the compiler to ignore the warning. Here is what happens line-by-line inside the function:
                    ...
                    -> The address of ptr we passed in is completely erased and forgotten.
